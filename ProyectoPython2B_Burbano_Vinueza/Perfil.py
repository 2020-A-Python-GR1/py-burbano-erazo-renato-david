...
class Perfil:
    ...
    def __init__(self, documento, nombre_usuario, correo, fecha_nacimiento, pais_usuario):
        self.codigo_documento = documento
        self.nombre_usuario_var = nombre_usuario
        self.correo = correo
        self.fecha_nacimiento_usuario = fecha_nacimiento
    ...
    def actualizar(self, nombre_usuario, correo, fecha_nacimiento, pais, advertencia, ventana):
        ...
        if (patternCorreo.fullmatch(correo) is not None):
            if (patternFecha.fullmatch(fecha_nacimiento) is not None):
                try:
                    ...
                    for documento in resultados_consulta:
                        usuario = resultados_consulta[documento]
                        if (nombre_usuario == usuario['nombre_usuario'] and nombre_usuario!=self.nombre_usuario_var):
                            advertencia.set("Error: El nombre de usuario seleccionado ya existe")
                            return
                        if(correo == usuario['correo'] and correo != self.correo):
                            advertencia.set("Error: El correo seleccionado esta registrado por otro usuario")
                            return
                                
                    firebase.put('/proyectopython2020a-d2866/Usuario/' + self.codigo_documento, 'nombre_usuario', nombre_usuario)
                    firebase.put('/proyectopython2020a-d2866/Usuario/' + self.codigo_documento, 'correo', correo)
                    firebase.put('/proyectopython2020a-d2866/Usuario/' + self.codigo_documento, 'fecha_nacimiento', fecha_nacimiento)
                    firebase.put('/proyectopython2020a-d2866/Usuario/' + self.codigo_documento, 'pais', pais)

                    self.nombre_usuario_var = nombre_usuario
                    self.correo = correo
                    self.fecha_nacimiento_usuario = fecha_nacimiento
                    self.pais_usuario = pais

                    messagebox.showinfo(message="La informacion ha sido actualizada exitosamente", title="Informacion")
